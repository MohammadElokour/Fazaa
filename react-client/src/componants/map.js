	...
	componentDidMount() {
		fetch('/main-mapm', {
			...
			.then((acuiredData) => {
				...
				if (acuiredData) {
					this.setState({
						locations: { lat: acuiredData.user[0].dest_Lat, lng: acuiredData.user[0].dest_Lng }
					});
			...
			if (users) {
				...
				users.users.forEach((user) => {
					if (user.dest_Lat !== null || user.dest_Lat !== undefined)
						usersClickedLocations = [
							...usersClickedLocations,
							{ name: user.username + ' clicked Location', lat: user.dest_Lat, lng: user.dest_Lng }
						];
				});
	...
	render() {
		...
		return (
			<div>
				...
				<div id="btnBox">
					<NavLink to="/driver">
						<button className="mapB" type="button">
							DRIVER
						</button>
					</NavLink>
					<NavLink to="/passenger">
						<button className="mapB" type="button" onClick={this.updateP.bind(this)}>
							PASSENGER
						</button>
					</NavLink>
					{/* <NavLink to='/homepage'>
